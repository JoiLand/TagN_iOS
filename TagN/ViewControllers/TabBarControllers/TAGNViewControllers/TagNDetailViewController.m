...
@interface TagNDetailViewController ()

@end

@implementation TagNDetailViewController
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ImageObj *objImage = self.m_objImageInfo.imageinfo_images[section];
...
{
    ImageObj *objImage = self.m_objImageInfo.imageinfo_images[indexPath.section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(self.m_bSelectionMode
       ...
       && (objImage.image_user_id.intValue == [GlobalService sharedInstance].user_me.user_id.integerValue   //image owner
           || self.m_objImageInfo.imageinfo_tag.tag_user_id.intValue == [GlobalService sharedInstance].user_me.user_id.intValue //tag owner
           )) {
           cell.m_imgSelected.hidden = NO;
           
           if([m_arySelected[indexPath.section] boolValue]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(objImage.image_is_uploaded.boolValue){
        if(self.m_bSelectionMode) {
            if(objImage.image_user_id.intValue == [GlobalService sharedInstance].user_me.user_id.integerValue   //image owner
               || self.m_objImageInfo.imageinfo_tag.tag_user_id.intValue == [GlobalService sharedInstance].user_me.user_id.intValue //tag owner
               ) {
                [m_arySelected replaceObjectAtIndex:indexPath.section withObject:[NSNumber numberWithBool:![m_arySelected[indexPath.section] boolValue]]];
                
                [self.m_cltPhoto reloadItemsAtIndexPaths:@[[NSIndexPath indexPathForRow:indexPath.section inSection:0]]];
                [self.m_tblPhoto reloadSections:[NSIndexSet indexSetWithIndex:indexPath.section]
        ...
        if(self.m_bSelectionMode) {
            if(objImage.image_user_id.intValue == [GlobalService sharedInstance].user_me.user_id.integerValue   //image owner
               || self.m_objImageInfo.imageinfo_tag.tag_user_id.intValue == [GlobalService sharedInstance].user_me.user_id.intValue //tag owner
                ...
                [self.m_tblPhoto reloadSections:[NSIndexSet indexSetWithIndex:indexPath.row]
                               withRowAnimation:UITableViewRowAnimationNone];
                [self setMenuViews];
            }
        } else {
            ...
            [self.m_tblPhoto scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:indexPath.row]
