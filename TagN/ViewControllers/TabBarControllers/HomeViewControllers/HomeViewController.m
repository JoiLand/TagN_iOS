...
- (void)viewDidLoad {
    ...
    [refresh addTarget:self action:@selector(onRefreshMyTagN:) forControlEvents:UIControlEventValueChanged];
    [self.m_tblMyTags addSubview:refresh];
...
- (void)onRefreshMyTagN:(UIRefreshControl *)refresh {
    m_nCurrentPageNum = 1;
    [[JHImageService sharedInstance] getMyTagNImagesWithId:[GlobalService sharedInstance].user_me.user_id
...
- (void)addMyPhotos:(NSNotification *)notification {
    if(notification.userInfo[@"image_obj"]) {   //uploaded photo
        ...
        for(int nIndex = 0; nIndex < [GlobalService sharedInstance].user_me.user_my_albums.count; nIndex++) {
            ImageInfoObj *objImageInfo = [GlobalService sharedInstance].user_me.user_my_albums[nIndex];
            
            if(objImageInfo.imageinfo_tag.tag_id.intValue == nTagId.intValue) {
                [self.m_tblMyTags reloadSections:[NSIndexSet indexSetWithIndex:nIndex] withRowAnimation:UITableViewRowAnimationNone];
                break;
            }
        }
    } else {    // add photo
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ImageInfoObj *objImageInfo = [GlobalService sharedInstance].user_me.user_my_albums[section];
...
{
    return [GlobalService sharedInstance].user_me.user_my_albums.count;
}
...
{
    ImageInfoObj *objImageInfo = [GlobalService sharedInstance].user_me.user_my_albums[indexPath.section];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HomeDetailViewController *homeDetailVC = [self.storyboard instantiateViewControllerWithIdentifier:@"HomeDetailViewController"];
    homeDetailVC.m_objImageInfo = objImageInfo;
    homeDetailVC.m_bSelectionMode = NO;
...
- (void)onClickBtnDropDown:(UIView *)sender onSection:(NSInteger)nSection {
    ImageInfoObj *objImageInfo = [GlobalService sharedInstance].user_me.user_my_albums[nSection];
    ...
    if(m_nSelectedSection == nSection) {
        ...
        [m_viewPopover showAtPoint:startPoint popoverPostion:DXPopoverPositionDown withContentView:viewDropDown inView:self.m_tblMyTags];
    }
}
...
- (void)onClickBtnDeleteImagesWithSection:(NSInteger)section {
    ...
    HomeDetailViewController *homeDetailVC = [self.storyboard instantiateViewControllerWithIdentifier:@"HomeDetailViewController"];
    homeDetailVC.m_objImageInfo = objImageInfo;
    homeDetailVC.m_bSelectionMode = YES;
    [self.navigationController pushViewController:homeDetailVC animated:YES];
}
...
{
    if (buttonIndex == 1) {
        ImageInfoObj *objImageInfo = [GlobalService sharedInstance].user_me.user_my_albums[m_nSelectedSection];
        [self hideDropDownMenu];
